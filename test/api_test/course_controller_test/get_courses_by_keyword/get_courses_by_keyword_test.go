package get_courses_by_keyword
...
func TestGetCoursesByKeywordSuccess(t *testing.T) {
	e := di.InitializedEchoServerForTest()

	keywordQuery := "?keyword=Intro%20To%20Entrepreneurship"
	urlRoute := router.HostURL + router.CoursesAPIRoute + keywordQuery
	...
	assert.Equal(t, 1, len(coursesResponse))
}
